...
function renderizarRecentes() {
  ...
  // Prioridade de dados:
  // 1) Itens internos com lastAccessed (Storage)
  // 2) Fallback para repos recentes do localStorage
  let recentesItens = [];
  ...
  if (recentesItens.length === 0) {
    recentesItens = reposRecentes.slice(0, 3).map(nome => ({
      tipo: "repo",
      repo: nome,
      titulo: nome,
      url: `${GITHUB_URL}/${nome}`,
  ...
  if (recentesItens.length === 0) {
    lista.innerHTML = '<li><span>...</span><b>Nenhum acesso recente</b></li>';
    return;
  }
