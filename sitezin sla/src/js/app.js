// ===== APP MODULE (Refatorado para exports) =====
...
const App = {
  ...
  pages: [
    ...
    { id: "__page-settings", title: "Settings", era: "settings", keywords: ["configuracoes", "configurações", "preferencias", "perfil", "profile", "backup", "exportar", "importar", "seguranca"] }
  ],
            ...
            html += items.map(item => {
              ...
              return '<a href="' + href + '" class="search-result-item" data-id="' + item.id + '"' + target + '>' +
                ...
                '<div class="search-result-info">' +
                  ...
                  '<small>' + this.escapeHtml(item.category) + (item.tags.length ? ' · ' + this.escapeHtml(item.tags.join(", ")) : '') + '</small>' +
                '</div>' +
              '</a>';
            }).join("");
          }
...
// Exportar como funções para uso em React
